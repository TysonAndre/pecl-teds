--TEST--
Teds\StrictTreeMap::fromPairs
--FILE--
<?php

try {
    Teds\StrictTreeMap::fromPairs(['first' => 'x']);
} catch (Throwable $e) {
    printf("Caught %s: %s\n", $e::class, $e->getMessage());
}
$it = Teds\StrictTreeMap::fromPairs([[0, 1], [2, 'a'], [1, 'b']]);
foreach ($it as $key => $value) {
    printf("key=%s value=%s\n", json_encode($key), json_encode($value));
}
$it = Teds\StrictTreeMap::fromPairs([['first', 'x'], [(object)['key' => 'value'], null]]);
foreach ($it as $key => $value) {
    printf("key=%s value=%s\n", json_encode($key), json_encode($value));
}
$pairsIterable = call_user_func(function () { yield new stdClass() => [strtolower('KEY'), strtolower('OTHER')]; });
$it = Teds\StrictTreeMap::fromPairs($pairsIterable);
foreach ($it as $key => $value) {
    printf("key=%s value=%s\n", json_encode($key), json_encode($value));
}
var_dump($it->toPairs());

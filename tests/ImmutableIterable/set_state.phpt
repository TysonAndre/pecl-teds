...
dump_repr(Teds\ImmutableIterable::__set_state([]));
try {
    Teds\ImmutableIterable::__set_state(['first' => 'x']);
} catch (Throwable $e) {
    printf("Caught %s: %s\n", $e::class, $e->getMessage());
}
$it = Teds\ImmutableIterable::__set_state([['first', 'x'], [(object)['key' => 'value'], null]]);

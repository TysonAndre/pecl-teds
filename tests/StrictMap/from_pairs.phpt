--TEST--
Teds\StrictMap::fromPairs
--FILE--
<?php

try {
    Teds\StrictMap::fromPairs(['first' => 'x']);
} catch (Throwable $e) {
    printf("Caught %s: %s\n", $e::class, $e->getMessage());
}
$it = Teds\StrictMap::fromPairs([['first', 'x'], [(object)['key' => 'value'], null]]);
...
$it = Teds\StrictMap::fromPairs($pairsIterable);
foreach ($it as $key => $value) {
    printf("key=%s value=%s\n", json_encode($key), json_encode($value));
}
var_dump($it->toPairs());

...
PHP_METHOD(Teds_StrictSet, toArray)
{
	ZEND_PARSE_PARAMETERS_NONE();
	teds_strictset_entries *const array = Z_STRICTSET_ENTRIES_P(ZEND_THIS);
	...
	TEDS_STRICTSET_FOREACH_VAL(array, val) {
		Z_TRY_ADDREF_P(val);
...
PHP_MINIT_FUNCTION(teds_strictset)
{
	...
	teds_ce_StrictSet = register_class_Teds_StrictSet(zend_ce_aggregate, teds_ce_Set, php_json_serializable_ce);

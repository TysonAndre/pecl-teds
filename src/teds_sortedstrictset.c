...
static zend_always_inline void teds_sortedstrictset_tree_rebalance_after_insert(teds_sortedstrictset_tree *tree, teds_sortedstrictset_node *node) {
	while (true) {
...
finish_insert:
				...
				teds_sortedstrictset_tree_rebalance_after_insert(tree, c);
...
#define Z_SORTEDSTRICTSET_TREE_P(zv) (&Z_SORTEDSTRICTSET_P((zv))->array)
...
PHP_METHOD(Teds_SortedStrictSet, toArray)
{
	ZEND_PARSE_PARAMETERS_NONE();
	teds_sortedstrictset_tree *const array = Z_SORTEDSTRICTSET_TREE_P(ZEND_THIS);
	...
	TEDS_SORTEDSTRICTSET_FOREACH_KEY(array, val) {
		Z_TRY_ADDREF_P(val);
...
PHP_MINIT_FUNCTION(teds_sortedstrictset)
{
	...
	teds_ce_SortedStrictSet = register_class_Teds_SortedStrictSet(zend_ce_aggregate, teds_ce_Set, php_json_serializable_ce);
